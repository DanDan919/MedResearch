...
When no validated evidence exists, MedResearch creates an explicit `InsufficientEvidence` report without calling the LLM. Persisted report claims remain narrative and traceable; no vote counting, automatic model selection, HKSJ inference, prediction interval, formal GRADE, formal risk-of-bias result, diagnosis, or treatment recommendation is produced.
...
Milestone 17 adds `FixedEffectQuantitativeStatisticalSynthesizer`, a deterministic Application read model over M15-compatible groups. V1 supports OR/RR/HR groups only, pools normalized log effects with generic inverse-variance fixed-effect weighting, back-transforms by exponentiation, and exposes the result to synthesis context without persisting a new database table. M19 estimates REML tau-squared as between-study variance foundation data. M22 now uses that same tau-squared estimate for a separate REML random-effects inverse-variance pooled estimate with a Wald standard-normal confidence interval. It still does not implement HKSJ, prediction intervals, tau-squared confidence intervals, forest plots, p-value pooling, vote counting, semantic harmonization, or formal evidence-certainty claims.
...
This value is exposed in `SynthesisContext.QuantitativeSyntheses` and the synthesis prompt as supplied MedResearch-computed context. M22 consumes it to compute random-effects weights `1/(vi + tau²)`, a random-effects pooled estimate, variance, SE, and Wald confidence interval. It does not change fixed-effect pooled estimates, fixed-effect weights, confidence intervals, Cochran's Q, df, or I-squared. The normal automated test suite uses deterministic fixtures, including BCG/metafor reference checks, and does not call external statistical services.
