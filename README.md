...
When no validated evidence exists, MedResearch creates an explicit `InsufficientEvidence` report without calling the LLM. Persisted report claims remain narrative and traceable; no vote counting, automatic model selection, modified/ad-hoc HKSJ, prediction interval, formal GRADE, formal risk-of-bias result, diagnosis, or treatment recommendation is produced.
