...
public sealed class QuantitativeEvidenceAssessor : IQuantitativeEvidenceAssessor
{
        ...
        ICollection<QuantitativeIneligibilityReason> reasons)
    {
        ...
        var z = InverseStandardNormal(0.5d + confidenceLevel / 2d);
    ...
    private static double InverseStandardNormal(double probability)
    {
        ...
        var qHigh = Math.Sqrt(-2d * Math.Log(1d - probability));
        return -(((((c[0] * qHigh + c[1]) * qHigh + c[2]) * qHigh + c[3]) * qHigh + c[4]) * qHigh + c[5])
            / ((((d[0] * qHigh + d[1]) * qHigh + d[2]) * qHigh + d[3]) * qHigh + 1d);
    }
